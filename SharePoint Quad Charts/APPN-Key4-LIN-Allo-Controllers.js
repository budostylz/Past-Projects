    ...
    try{
        $( document ).ready(function() {
                   ...
                   var bosRootSourceData = $q.all([
                    ...
                    bosRootSourceData.then(function (bosRootData) {//BOS-ROOT SOURCE DATA
                        ...
                        if(checked){
                            ...
                            var createAPPNUFR = $SourceDataDOMService.createProgramAPPNUFR(sourceData, bos, root, vm.BOSRoot, vm.BOSRootUFR, vm.fys);//set Appropriations framework
                            ...
                            var setRDTEUFR = $SourceDataDOMService.APPNDOM(sourceData[bos]['RDTEUFR'], vm.fys, 'rdteufrfy', 'rdteufrPOMPeriod');
                            var setPROCUFR = $SourceDataDOMService.APPNDOM(sourceData[bos]['PROCUFR'], vm.fys, 'procufrfy', 'procufrPOMPeriod');
                            var setOTHERUFR = $SourceDataDOMService.APPNDOM(sourceData[bos]['OTHERUFR'], vm.fys, 'otherufrfy', 'otherufrPOMPeriod');
                            var setTOTALUFR = $SourceDataDOMService.APPNDOM(sourceData[bos]['TOTALUFR'], vm.fys, 'totalufrfy', 'totalufrPOMPeriod');
                            ...
                            var resetAPPN = $SourceDataDOMService.resetAPPNs_Key4s_Quantity_Inventories(sourceData, bos, root, vm.fys);//reset appropriations, UFRs, key4s, Quantities, Inventories
                            var rdteKey4s = $SourceDataDOMService.Key4DOM(vm.key4s, root, sourceData[bos]['SelectedRoots'][root]['RDTE']['Key4s'], checked);//set RDTE Key 4  
                            var procKey4s = $SourceDataDOMService.Key4DOM(vm.key4s, root, sourceData[bos]['SelectedRoots'][root]['PROC']['Key4s'], checked);//set PROC Key 4
                            var otherKey4s = $SourceDataDOMService.Key4DOM(vm.key4s, root, sourceData[bos]['SelectedRoots'][root]['OTHER']['Key4s'], checked);//set OTHER Key 4
                            var setLINs = $SourceDataDOMService.LINDOM(vm.LINs, root, sourceData[bos]['SelectedRoots'][root]['Quantity']['LINs'], checked);//set LIN DOM
                ...
                jQuery("#LIN").on( "click", "input", function(e) {//LIN
                    ...
                    var area = sourceData.Area;
                    var bos = sourceData.BOS;
                    var portfolio = sourceData.PORTFOLIO;
                    var program = sourceData.Program;
                   ...
                   //AAO
                    var AAOUrl = $GetItemsService.AAOUrl(portfolio, LIN);
                    var AAOItems = $GetItemsService.getItems(AAOUrl);

                   //APO
                   var APOUrl = $GetItemsService.APOUrl(portfolio, LIN);
                   var APOItems = $GetItemsService.getItems(APOUrl);


                  //INVENTORY
                  var InventoryUrl = $GetItemsService.ONHANDUrl(portfolio, LIN);
                  var InventoryItems = $GetItemsService.getItems(InventoryUrl);



                  var LIN_Data = $q.all([fyItems, 
                                         AAOItems,
                                         APOItems,
                                         InventoryItems
                    ...
                    LIN_Data.then(function (LINData) {//BOS-ROOT SOURCE DATA
                        ...
                        var defineAAOAPO = $SourceDataDOMService.defineAAO_APO_ONHAND(bos, root, LIN, vm.AAO, vm.APO, vm.Inventory, sourceData, checked);//define APO/AAO/Inventory
