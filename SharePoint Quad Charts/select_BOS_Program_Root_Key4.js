...
angular.module('myApp').factory('GetDataService', function ($q) {
    ...
    return {
        ...
        getData2:function(bos){
           ...
           var defer = $q.defer();
           var BOSRootEndPoint = ctx.webAbsoluteUrl + "/_api/web/lists/getbytitle('Programs')/items?$top=5000&select=Root&$filter=BOS eq '"+encodeURIComponent(bos)+"'";
...
angular.module('myApp').controller('MainController', ['$q', 'GetDataService', function ($q, $GetDataService) {
     ...
     var recursiveTest = $GetDataService.recursiveTest();
    ...
    completed.then(function (data) {
      ...
      //load Portfolios
        for(var i = 0; i < vm.BOSName.length; i++){      
          ...
          jQuery("#bos").append('<option>'+_bosname+'</option>');//add portfolio names to drop down
          //console.log(_bosname);    
        }  
